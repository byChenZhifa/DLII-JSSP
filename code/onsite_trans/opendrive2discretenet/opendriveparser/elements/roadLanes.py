...
from onsite_trans.opendrive2discretenet.opendriveparser.elements.road_record import RoadRecord
...
class LeftLanes:
    ...
    def lanes(self):
        """ """
        self._lanes.sort(key=lambda x: x.id, reverse=self.sort_direction)  # Lane ids in the left lane group are in ascending order
        return self._lanes
...
class RightLanes(LeftLanes):
    ...
    sort_direction = True  # Lane ids in the right lane group are in descending order
...
class LaneLink:
    """Records the lane connection and stores the id corresponding to the leading lane and the subsequent lane
    [Different from the connection relation of road and road,laneLink only contains the id of the corresponding lane,
    while the connection of road contains the type\id\contactPoint of the corresponding road]
    """
