...
from onsite_trans.opendrive2discretenet.plane_elements.plane import (
    ParametricLane,
    ParametricLaneBorderGroup,
)
from onsite_trans.opendrive2discretenet.plane_elements.plane_group import ParametricLaneGroup
from onsite_trans.opendrive2discretenet.plane_elements.border import Border
...
class OpenDriveConverter:
    ...
    def create_reference_border(plan_view, lane_offsets) -> Border:
        ...
        if any(lane_offsets):
            ...
            for lane_offset in lane_offsets:
                ...
                reference_border.width_coefficients.append(lane_offset.polynomial_coefficients)  # List[coefficients]
        else:
    ...
    def lane_section_to_parametric_lanes(lane_section, reference_border) -> List[ParametricLaneGroup]:
        ...
        for side in ["right", "left"]:
            ...
            for lane in lanes:
                inner_neighbour_id, outer_neighbour_id, inner_neighbour_same_dir = OpenDriveConverter.determine_neighbours(lane)

                outer_parametric_lane_border = OpenDriveConverter._create_outer_lane_border(lane_borders, lane, coeff_factor)
                ...
                for width in lane.widths:
                    parametric_lane = OpenDriveConverter.create_parametric_lane(lane_borders, width, lane)
    ...
    def determine_neighbours(lane) -> Tuple[str, str, bool]:
        ...
        inner_neighbour_id = encode_road_section_lane_width_id(lane.lane_section.parentRoad.id, lane.lane_section.idx, inner_lane_id, -1)

        outer_neighbour_id = encode_road_section_lane_width_id(lane.lane_section.parentRoad.id, lane.lane_section.idx, outer_lane_id, -1)
