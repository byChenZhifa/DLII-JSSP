...
import numpy as np
from onsite_trans.opendrive2discretenet.discrete_network import DiscreteLane
...
class ParametricLaneGroup:
    ...
    def to_discretelane(self, precision: float = 0.5) -> DiscreteLane:
        ...
        for parametric_lane in self.parametric_lanes:
            ...
            local_left_vertices, local_right_vertices = parametric_lane.calc_vertices(precision=precision)
        ...
        lanelet = DiscreteLane(self, left_vertices, center_vertices, right_vertices, self.id_)
    ...
    def first_zero_width_change_position(self, reverse: bool = False, reference_width: float = 0.0) -> float:
    ...
    def first_width_change_reversal_point(self, reverse: bool = False) -> float:
        ...
        for i in range(len(self.parametric_lanes) - 1):
            ...
            if first_plane.calc_width(0.99 * first_plane.length) > second_plane.calc_width(0 + 0.01 * second_plane.length):
                between_plane_width_dev_changes.append(first_plane.length + self._geo_lengths[i])
